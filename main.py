...
def create_event():
    ...
    start = data['start'] + ":00.000Z" # adjusting the format of dates for the google calendar event
    end = data['end'] + ":00.000Z"
    ...
    try:
        # make the google calendar event with the initial attendees
        service = get_google_calendar_service()
        created_event = service.events().insert(calendarId="primary", body=event).execute()
        event_id = created_event.get('id')
        event_link = created_event.get('htmlLink') 

        # create the event, pass the event_id and event_link
